...
extern "C" {
    ...
    void dyn_file_append_all(const char* path, const char* content) {
        std::ofstream file(fs::path(path), std::ios::out | std::ios::app | std::ios::binary);
        if (!file) {
            last_file_error = 2; // FAILED
            return;
        }
    ...
    int64_t dyn_file_open(const char* path, const char* mode) {
        ...
        if (std::strcmp(mode, "r") == 0) std_mode |= std::ios::in;
        else if (std::strcmp(mode, "w") == 0) std_mode |= std::ios::out;
        else if (std::strcmp(mode, "a") == 0) std_mode |= (std::ios::out | std::ios::app);
    ...
    const char* dyn_file_read_line(int64_t raw_handle) {
        if (!raw_handle) return alloc_gc_string("");
        auto *handle = reinterpret_cast<FileHandle *>(raw_handle);
