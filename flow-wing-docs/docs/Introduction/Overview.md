...
**Flow-Wing** (`.fg` sources) builds on LLVM. One compiler project gives you two tools: an **ahead-of-time (AOT)** binary that compiles to a native executable, and a separate **just-in-time (JIT)** binary that runs programs in one step. The runtime uses a built-in garbage collector for heap data — you never pair allocations with manual `free`.
