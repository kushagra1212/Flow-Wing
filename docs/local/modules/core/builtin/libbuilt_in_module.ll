...
define   i8* @concat_strings(i8* %str1, i8* %str2) {
    ...
    %concatStr = call i8* @GC_malloc(i64 %totalLenPlusOne_i64)
...
define  i8* @itos(i32 %num) {
    ...
    %buffer = call i8* @GC_malloc(i64 %int_64_num)
...
; ; Override malloc with GC_malloc
; define  i8* @malloc(i64 %num) {
;     %buffer = call i8* @GC_malloc(i64 %num)
;     ret i8* %buffer
; }
...
;     %buffer = call i8* @GC_malloc(i64 %totalSize)
;     ret i8* %buffer
; }
...
define  i8* @dtos(double %f) {
  ; Allocate memory for the string buffer
  %buffer = call i8* @GC_malloc(i64 64)
...
define  i8* @get_malloc_ptr_of_string_constant(i8* %str) {
    ...
    %strPtr = call i8* @GC_malloc(i64 %lenIncreasedByOne_64)
...
define  i8* @get_malloc_ptr_of_int_constant(i32 %num) {
    ; Allocate memory for the string buffer
    %buffer = call i8* @GC_malloc(i64 12)
...
entry:
  ...
  %inputValue = call i8* @GC_malloc(i64 1000001)
