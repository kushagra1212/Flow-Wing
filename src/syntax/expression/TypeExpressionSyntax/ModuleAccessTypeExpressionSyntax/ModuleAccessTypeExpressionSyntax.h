...
class ModuleAccessTypeExpressionSyntax : public ExpressionSyntax {

public:
  ModuleAccessTypeExpressionSyntax(
  ...
  getModuleIdentifier() const;
  const std::unique_ptr<ExpressionSyntax> &getTypeExpression() const;

private:
  std::unique_ptr<IdentifierExpressionSyntax> m_module_identifier_expression;
