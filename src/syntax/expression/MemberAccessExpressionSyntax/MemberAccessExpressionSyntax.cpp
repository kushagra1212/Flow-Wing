...
MemberAccessExpressionSyntax::MemberAccessExpressionSyntax(
    ...
    : m_left_expression(std::move(left_expression)), m_dot_token(dot_token),
      m_right_expression(std::move(right_expression)) {}

// Overrides
NodeKind MemberAccessExpressionSyntax::getKind() const {
  return NodeKind::kMemberAccessExpression;
}

void MemberAccessExpressionSyntax::accept(visitor::ASTVisitor *visitor) {
  visitor->visit(this);
}

const std::unique_ptr<ExpressionSyntax> &
MemberAccessExpressionSyntax::getLeftExpression() const {
  return m_left_expression;
}

const std::unique_ptr<ExpressionSyntax> &
MemberAccessExpressionSyntax::getRightExpression() const {
  return m_right_expression;
}

const std::vector<const SyntaxNode *> &
MemberAccessExpressionSyntax::getChildren() const {
  if (m_children.empty()) {
    for (const auto &node :
         {static_cast<const SyntaxNode *>(m_left_expression.get()),
          static_cast<const SyntaxNode *>(m_dot_token),
