...
template <typename T>
const int &SyntaxToken<T>::getColumnNumber() {
  return this->columnNumber;
}

template <typename T>
const int &SyntaxToken<T>::getLineNumber() {
  return this->lineNumber;
}

template <typename T>
const std::string &SyntaxToken<T>::getText() {
  return this->text;
}
...
template <typename T>
const T &SyntaxToken<T>::getValue() {
  return (this->value);
}
template <typename T>
std::string SyntaxToken<T>::getKindText() {
  return SyntaxKindUtils::to_string(this->kind);
}
template <typename T>
const int SyntaxToken<T>::getUnaryOperatorPrecedence() {
  switch (this->kind) {
...
template <typename T>
const int SyntaxToken<T>::getBinaryOperatorPrecedence() {
  switch (this->kind) {
...
template <typename T>
const DiagnosticUtils::SourceLocation SyntaxToken<T>::getSourceLocation()
    const {
